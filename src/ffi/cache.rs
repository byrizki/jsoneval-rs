...
#[no_mangle]
pub unsafe extern "C" fn json_eval_cache_stats(handle: *mut JSONEvalHandle) -> FFIResult {
    if handle.is_null() {
...
/// - handle must be a valid pointer from json_eval_new
#[no_mangle]
pub unsafe extern "C" fn json_eval_clear_cache(handle: *mut JSONEvalHandle) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error("Invalid handle pointer".to_string());
    }
...
/// - Caller must call json_eval_free_result when done
#[no_mangle]
pub unsafe extern "C" fn json_eval_cache_len(handle: *mut JSONEvalHandle) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error("Invalid handle pointer".to_string());
    }
...
/// - handle must be a valid pointer from json_eval_new
#[no_mangle]
pub unsafe extern "C" fn json_eval_enable_cache(handle: *mut JSONEvalHandle) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error("Invalid handle pointer".to_string());
    }
...
/// - handle must be a valid pointer from json_eval_new
#[no_mangle]
pub unsafe extern "C" fn json_eval_disable_cache(handle: *mut JSONEvalHandle) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error("Invalid handle pointer".to_string());
    }
...
#[no_mangle]
pub unsafe extern "C" fn json_eval_is_cache_enabled(handle: *mut JSONEvalHandle) -> i32 {
    if handle.is_null() {
        return 0;
    }

    let eval = &(*handle).inner;
    if eval.is_cache_enabled() {
        1
    } else {
        0
    }
