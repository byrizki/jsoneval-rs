    ...
    fn try_get_cached(&self, eval_key: &str, eval_data: &EvalData) -> Option<Value> {
        ...
        let cache_key = if deps.is_empty() {
            ...
            let dep_values: Vec<(String, &Value)> = filtered_deps
                .iter()
                .filter_map(|dep_key| eval_data.get(dep_key).map(|v| (dep_key.clone(), v)))
                .collect();
        ...
        hidden_fields: &mut Vec<String>,
         ...
         match schema_element {
            Value::Object(map) => {
                ...
                // Check extra_comments debug removed
