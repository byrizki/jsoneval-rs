        ...
        match row_idx_expr {
            CompiledLogic::IndexAt(lookup_expr, idx_table_expr, field_expr, range_expr) => {
                ...
                if self.tables_match_fast(table_expr, idx_table_expr) {
                    ...
                    if self.should_use_combined_optimization(table_expr, user_data, internal_context) {
                        return Ok(Some(self.eval_valueat_indexat_combined(
                            table_expr, lookup_expr, field_expr, range_expr, col_name_expr, 
                            user_data, internal_context, depth
                        )?));
                    }
                }
            }
                ...
                if !conditions.is_empty() && self.tables_match_fast(table_expr, fi_table_expr) {
                    if self.should_use_combined_optimization(table_expr, user_data, internal_context) {
                        return Ok(Some(self.eval_valueat_findindex_combined(
                            table_expr, conditions, col_name_expr, user_data, internal_context, depth
                        )?));
                    }
                }
            }
        ...
        if let Some(arr) = table_ref.as_array() {
            for row in arr.iter() {
                ...
                for condition in conditions {
                    ...
                    let result = self.evaluate_with_context(condition, user_data, row, depth + 1)?;
                    if !is_truthy(&result) {
                        all_match = false;
                        break;
                    }
