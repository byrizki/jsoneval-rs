...
description: "The ParsedSchemaCache is a built-in, thread-safe cache for storing and reusing Arc<ParsedSchema> instances. This enables high-performance scenarios where schemas are parsed once and reused across multiple evaluations."
navigation:
  icon: 'i-ph-book-open'
seo:
  title: 'Parsed Schema Cache'
...
::code-group
  ...
  // Pre-load schemas into the cache in your native setup phase.
  ```
  ```typescript [React Native]
  // ParsedSchemaCache is managed natively.
  // Requires schema to be pre-loaded in the Native environment.
  ```
...
::code-group
  ...
  ```
  ```csharp [C#]
  // Cache removal is managed by the Rust core bindings.
  ```
  ```typescript [React Native]
  // Cache removal is managed by the native core runtime.
  ```
  ```typescript [Web / NodeJS]
  // Cache removal is managed by the native core runtime.
  ```
...
::code-group
  ...
  ```
  ```csharp [C#]
  // Cache clearing is managed by the Rust core bindings.
  ```
  ```typescript [React Native]
  // Cache clearing is managed by the native core runtime.
  ```
  ```typescript [Web / NodeJS]
  // Cache clearing is managed by the native core runtime.
  ```
