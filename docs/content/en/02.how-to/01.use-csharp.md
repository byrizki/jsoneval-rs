---
title: 'C#'
description: "Use the JsonEvalRs NuGet package to execute the JSONEval engine natively in .NET."
navigation:
...
Updates internal engine states without converting the output logic tree back to C# JObject structures.
...
Parses and executes a raw JSON Logic block directly outside standard schemas.
```csharp
...
// Returns the evaluated schema out of the Rust memory space as a raw MessagePack byte array
...
**Schema Reloading**
Replaces the underlying engine schema tree over an existing evaluated node state without triggering heavy object reinstantiations.
```csharp
