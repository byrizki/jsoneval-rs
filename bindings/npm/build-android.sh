...
for abi in "${BUILD_ABIS[@]}"; do
    ...
    if [ -f "$SO_PATH" ]; then
        ...
        JNILIBS_DIR="bindings/npm/packages/react-native/android/src/main/jniLibs/$abi"
        mkdir -p "$JNILIBS_DIR"
...
for abi in "${BUILD_ABIS[@]}"; do
    echo "  $abi: bindings/npm/packages/react-native/android/src/main/jniLibs/$abi/libjson_eval_rs.so"
done
echo ""
echo "These libraries are bundled with the npm package."
echo ""
echo "Now you can build the React Native Android app:"
echo "  cd bindings/npm/examples/rncli/android"
