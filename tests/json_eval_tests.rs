...
fn test_validate_all_rules_pass() {
    ...
    let validation = eval
        .validate(&data, None, None, None, None, None)
...
fn test_validate_required_field_missing() {
    ...
    assert_eq!(
        ...
        "Error message should match schema"
    );
}
...
fn test_validate_min_max_value() {
    ...
    let validation = eval
        .validate(&data_min_str, None, None, None, None, None)
        .expect("Validation failed");
    ...
    assert_eq!(
        error.message, "Age must be at least 1",
        "Error message should match schema"
    );
    ...
    let validation2 = eval2
        .validate(&data_max_str, None, None, None, None, None)
        .expect("Validation failed");

    assert!(
        validation2.has_error,
    ...
    assert_eq!(
        error2.message, "Age must be at most 100",
        "Error message should match schema"
    );
...
fn test_validate_with_path_filter() {
    ...
    assert!(
        validation_filtered.has_error,
        "Should have validation error for age"
    );
    assert_eq!(
        validation_filtered.errors.len(),
        1,
...
fn test_evaluate_dependents_basic() {
    ...
    let mut eval =
        JSONEval::new(&schema, None, Some(&initial_data)).expect("Failed to create JSONEval");
    ...
    assert_eq!(
        changes.len(),
        1,
        "Should have exactly 1 dependent change (age)"
    );

    let age_change = &changes[0];
    assert!(
        age_change["$ref"].as_str().unwrap().contains("age"),
        "Dependent should be age field"
    );
    assert_eq!(
        age_change["transitive"].as_bool(),
        Some(false),
    ...
    assert!(
        age_change.get("value").is_some() || age_change.get("$field").is_some(),
...
fn test_evaluate_dependents_with_clear() {
    ...
    let mut eval =
        JSONEval::new(&schema, None, Some(&initial_data_str)).expect("Failed to create JSONEval");
...
fn test_evaluate_dependents_transitive() {
    ...
    assert_eq!(
        risk_change["transitive"].as_bool(),
        Some(true),
        "risk_category should be transitive dependent"
    );
    assert_eq!(
        risk_change.get("value").and_then(|v| v.as_str()),
        Some("Medium"),
...
fn test_evaluate_dependents_no_data_update() {
    ...
    assert_eq!(
        change["value"], 21,
        "Should compute using existing data: 7 * 3 = 21"
    );
}
...
fn test_evaluate_dependents_output_structure() {
    ...
    assert_eq!(
        field.get("type"),
        Some(&json!("number")),
        "$field should contain field schema"
    );
    assert_eq!(field.get("title"), Some(&json!("Computed Field")));

    // 3. Validate $parentField (the actual parent data object, not schema)
    assert!(
        value_change.get("$parentField").is_some(),
        "Must have $parentField"
    );
    assert!(
        value_change["$parentField"].is_object(),
        "$parentField must be an object"
    );
    ...
    assert!(
        parent_field.contains_key("type"),
        "$parentField should contain type data"
    );

    // 4. Validate value field
    assert!(value_change.get("value").is_some(), "Must have value field");
    assert_eq!(
        value_change["value"], 60,
        "Value should be trigger * 5 = 60"
    );

    // 5. Validate transitive field
    assert!(
        value_change.get("transitive").is_some(),
        "Must have transitive field"
    );
    assert!(
        value_change["transitive"].is_boolean(),
        "transitive must be a boolean"
    );
    assert_eq!(
        value_change["transitive"], false,
        "Direct dependent should have transitive=false"
    );

    // 6. Validate clear field is not present (only when field is cleared)
    assert!(
        value_change.get("clear").is_none(),
        "clear should not be present when not clearing"
    );
    ...
    assert_eq!(
        parent_field2.get("type"),
        Some(&json!("object")),
        "Parent should contain type data"
    );
    ...
    assert!(
        clear_change.get("clear").is_some(),
        "Must have clear field when clearing"
    );
    assert!(
        clear_change["clear"].is_boolean(),
        "clear must be a boolean"
    );
    assert_eq!(
        clear_change["clear"], true,
        "Field should be cleared (trigger > 10)"
    );

    // 6. Validate value field is not present (only when computing value)
    assert!(
        clear_change.get("value").is_none(),
        "value should not be present when only clearing"
    );
...
fn test_evaluate_dependents_dot_vs_schema_path() {
    ...
    let mut eval1 = JSONEval::new(&schema, None, Some(&data)).expect("Failed to create JSONEval");
    eval1
        .evaluate(&data, None, None, None)
        .expect("Initial evaluation failed");
    ...
    let mut eval2 = JSONEval::new(&schema, None, Some(&data)).expect("Failed to create JSONEval");
    eval2
        .evaluate(&data, None, None, None)
        .expect("Initial evaluation failed");
    ...
    println!(
        "✅ Both formats work identically! {} changes each",
        changes1.len()
    );
}
