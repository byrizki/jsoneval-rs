...
#[cfg(test)]
mod tests {
    use json_eval_rs::ffi::core::{json_eval_free, json_eval_free_result, json_eval_new};
    ...
    fn test_ffi_validate_integrity() {
        unsafe {
            ...
            let result = json_eval_validate(handle, data_cstr.as_ptr(), std::ptr::null(), false, false);
            ...
            let result_paths = json_eval_validate_paths(
                handle,
                data_cstr.as_ptr(),
                std::ptr::null(),
                paths_cstr.as_ptr(),
            ...
            let slice_paths =
                std::slice::from_raw_parts(result_paths.data_ptr, result_paths.data_len as usize);
