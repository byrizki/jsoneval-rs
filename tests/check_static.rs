...
fn test_get_evaluated_schema_resolves_all_static_arrays_minimal() {
    ...
    // Programmatically inflate the RATES array to >10 items so it gets extracted as a $static_array
    if let Some(rates) = schema_json
        .pointer_mut("/$params/references/RATES")
    ...
    let rates = full_schema.pointer("/$params/references/RATES");

    assert!(
        rates.is_some(),
        "RATES should exist in get_evaluated_schema"
    );
    let rates_val = rates.unwrap();

    assert!(
        rates_val.is_array(),
        "RATES should be an array resolved from $static_array marker, but got {:?}",
        rates_val
    );

    // Additional basic checks to ensure it's the expected array
    let arr = rates_val.as_array().unwrap();
    assert!(
        arr.len() > 200,
