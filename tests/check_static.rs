...
fn test_get_evaluated_schema_resolves_all_static_arrays_minimal() {
    ...
    let mut eval = JSONEval::new(&inflated_schema_str, None, None).expect("Should initialize successfully");

    // Perform a first evaluation without any input data
    eval.evaluate("{}", None, None, None).expect("Should evaluate successfully");
    ...
    assert!(premium_rates.is_some(), "PREMIUM_RATES should exist in get_evaluated_schema");
    ...
    assert!(arr.len() > 200, "Should contain the programmatically inflated rows (200+)");
    
    let first_row = &arr[0];
    assert_eq!(first_row.get("AGE").and_then(Value::as_i64), Some(20), "First row should have AGE: 20");
}
